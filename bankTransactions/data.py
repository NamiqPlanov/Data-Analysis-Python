...
#Identify missing values in each column and handle them (e.g., impute, drop).
#print('Is there any duplicated row?{}'.format(data.isnull().values.any()))

#Calculate the mean, median, and standard deviation for TransactionAmount, CustomerAge, and AccountBalance
...
#Plot histograms for TransactionAmount, CustomerAge, and TransactionDuration.
'''
...
plt.show()
'''
#Group transactions by TransactionDate and calculate daily total transaction amounts.
...
#Identify the top 10 locations by the number of transactions.
#1st method
topLocations = data['Location'].value_counts().head(10)
#print(topLocations)

topLocations2 = data.groupby(['Location','TransactionType']).size().reset_index(name='TotalTransactions')
sortedlocations = topLocations2.nlargest(10,'TotalTransactions')
...
#Find the average TransactionAmount per customer.

avgtransaction = data.groupby('TransactionID')['TransactionAmount'].mean().reset_index().head(15)
#print(avgtransaction)

#Extract the hour from TransactionDate and identify peak transaction hours.
...
#Create a pie chart showing the distribution of TransactionType
...
#Analyze the popularity of each Channel
...
#sns.heatmap(specificCols,cmap='Blues')
#plt.show()

#Identify accounts with unusually high LoginAttempts.
maxAttempts = 3
...
#Detect transactions with unusually long or short durations
minDuration = 150
maxDuration = 260
...
#Identify customers with transactions from multiple IP addresses.
...
customersWithMultipleIPs = customersWithUniqueIPs[customersWithUniqueIPs['DistinctIPCount']>1]['AccountID'].head(20)
#print(customersWithMultipleIPs)


#Segment customers based on their CustomerAge, TransactionAmount
...
#Identify the top merchants by transaction volume

topMerchants = data.nlargest(5,'TransactionAmount')[['TransactionID','AccountID','TransactionAmount']]
#print(topMerchants)


#Calculate the average transaction amount per day for each account.
AvgTransaction = data.groupby('TransactionDay')['TransactionAmount'].mean().reset_index()
#print(AvgTransaction)



#Analyze how AccountBalance changes after each transaction.
...
#Identify which devices are most commonly used for transactions.
